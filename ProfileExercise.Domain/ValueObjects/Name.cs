...
public sealed class Name : ValueObject
{
    ...
    private static readonly Regex ValidNameRegex =
        new(@"^[A-Za-zÀ-ÖØ-öø-ÿ\-'\s]{1,50}$", RegexOptions.Compiled);
    ...
    public Name(string value)
    {
        ...
        if (collapsed.Length > MaxLength)
            throw new ArgumentException($"Name must be at most {MaxLength} characters.", nameof(value));

        if (!ValidNameRegex.IsMatch(collapsed))
            throw new ArgumentException(
                "Name may only contain letters, spaces, hyphens, or apostrophes.",
                nameof(value));

        Value = CultureInfo.InvariantCulture.TextInfo.ToTitleCase(collapsed.ToLowerInvariant());
    }
